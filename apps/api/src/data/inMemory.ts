import { Listing, ListingImage } from '@threadloop/shared';
import { randomUUID } from 'crypto';

type ListingImageInput = Pick<ListingImage, 'storageUrl' | 'qualityScore'> &
  Partial<Pick<ListingImage, 'id' | 'listingId'>>;

export type ListingInput = Omit<Listing, 'id' | 'images'> & {
...
const staticUrl = (fileName: string) => `${staticBasePath}/${fileName}`;

const sampleListings: ListingInput[] = [
...
function hydrateListing(partial: ListingInput, overrideId?: string): Listing {
  const listingId = overrideId ?? randomUUID();
  return {
    ...partial,
    id: listingId,
...
export function createListing(partial: ListingInput): Listing {
